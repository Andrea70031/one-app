...
begin
  ...
  insert into public.daily_reports (
    site_id, report_date, summary, workers, hours, works, blockers, created_by
  ) values (
    p_site_id,
  ...
  loop
    if nullif(trim(v_item ->> 'title'), '') is null then continue; end if;
  ...
  loop
    if nullif(trim(v_item ->> 'title'), '') is null then continue; end if;
    insert into public.issues (site_id, title, details, priority, status, due_at, created_by)
    values (
      p_site_id,
      left(trim(v_item ->> 'title'), 180),
      nullif(left(trim(v_item ->> 'details'), 1600), ''),
      case when v_item ->> 'priority' in ('Bassa', 'Media', 'Alta', 'Critica') then v_item ->> 'priority' else 'Media' end,
      'Aperto',
  ...
  if p_progress is not null then
    update public.sites set progress = p_progress where id = p_site_id;
