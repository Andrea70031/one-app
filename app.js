...
async function logAction(a,status="executed"){if(!session)return;try{await cloudRequest("one_actions",{method:"POST",body:{user_id:userId(),kind:a.kind,label:a.label,status,payload:a.payload||{},executed_at:status==="executed"?new Date().toISOString():null}})}catch{}}
