...
const WORKSPACE_ACTION_KINDS=new Set(["create_site","create_issue","create_activity","create_daily_report","update_site_progress"]);
const EXAMPLE_PACKS={
  residential:{icon:"⌂",name:"Cantiere residenziale",description:"Una villetta in costruzione con avanzamento, attività, criticità e giornale lavori.",site:{name:"Villa Aurora · Demo",client:"Famiglia Rossi",address:"Via dei Tigli 18 · Milano",progress:35,notes:"Pacchetto dimostrativo ONE · Cantiere residenziale"},activities:[{title:"Getto solaio piano primo",notes:"Getto completato e maturazione in corso."},{title:"Consegna laterizi",notes:"Materiale ricevuto e verificato in area deposito."}],issues:[{title:"Verifica quota vano scala",details:"Controllare la quota prima della posa del primo gradino.",priority:"Alta",dueDays:1},{title:"Confermare colore infissi",details:"Attesa conferma definitiva del cliente.",priority:"Media",dueDays:3}],report:{summary:"Struttura del piano primo completata",workers:7,hours:56,works:"Getto solaio, controllo armature e ricezione laterizi.",blockers:"In attesa della conferma sul colore degli infissi."}},
  office:{icon:"▦",name:"Ristrutturazione ufficio",description:"Una commessa interna con impianti, finiture, consegne e una criticità aperta.",site:{name:"Uffici Polaris · Demo",client:"Polaris S.r.l.",address:"Viale Europa 42 · Bologna",progress:62,notes:"Pacchetto dimostrativo ONE · Ristrutturazione ufficio"},activities:[{title:"Posa pareti vetrate",notes:"Completata l'ala nord; resta la sala riunioni."},{title:"Collaudo rete dati",notes:"Testate 38 prese su 42."}],issues:[{title:"Ritardo corpi illuminanti",details:"Il fornitore ha spostato la consegna di quattro giorni.",priority:"Alta",dueDays:2}],report:{summary:"Finiture e impianti in avanzamento",workers:5,hours:40,works:"Pareti vetrate, cablaggio dati e preparazione controsoffitti.",blockers:"Consegna corpi illuminanti da ripianificare."}}
};
...
async function syncFromCloud(){
  ...
  try{
    ...
    if(Array.isArray(rows)&&rows.length){
      const a=rows.map(x=>({id:x.id,icon:x.icon||"✦",title:x.title,detail:x.detail||"",time:new Date(x.created_at).toLocaleDateString("it-IT",{day:"2-digit",month:"2-digit"}),type:x.type||"activity",payload:x.payload||{},cloud:true}));
...
async function addActivity(title,detail,icon="✦",type="ai",payload={}){
  const a=getLocalActivities();const local={id:Date.now().toString(),icon,title:title.slice(0,100),detail:detail.slice(0,180),time:"Ora",type,payload};a.unshift(local);saveLocalActivities(a);renderActivities();
  if(session){
...
async function saveMemory(result,sourceType="ai",sourceName=""){
  ...
  try{await cloudRequest("one_memories",{method:"POST",body:{user_id:userId(),site_id:selectedSiteId||null,kind:result?.type||"note",title:title.slice(0,180),summary:summary.slice(0,700),source_type:sourceType,source_name:sourceName,payload:{extracted:result?.extracted||{},answer:result?.summary||""}}});toast("Salvato in Recall cloud");await loadCloudMemories()}catch{toast("Non riesco a salvare in Recall cloud")}
}
async function loadCloudMemories(){
  if(!session)return;
  try{
    const rows=await cloudRequest("one_memories?select=*&order=created_at.desc&limit=40");
    $("recallList").innerHTML=rows.length?rows.map((x,index)=>`<button class="activity-card" data-memory="${index}"><span class="a-icon">◌</span><span class="a-copy"><strong>${escapeHtml(x.title)}</strong><small>${escapeHtml(x.summary||"")}</small></span><time>${new Date(x.created_at).toLocaleDateString("it-IT",{day:"2-digit",month:"2-digit"})}</time></button>`).join(""):`<div class="empty-state">Recall cloud è vuoto. Chiedi a ONE di analizzare qualcosa e salvalo.</div>`;
    $$("[data-memory]").forEach(button=>button.onclick=()=>{const memory=rows[Number(button.dataset.memory)];if(memory)openSheet("activity",{title:memory.title,detail:memory.payload?.answer||memory.summary||"",cloud:true})})
  }catch{}
}
...
function bindActivityClicks(){$$("[data-activity]").forEach(b=>b.onclick=()=>{const a=getLocalActivities().find(x=>x.id===b.dataset.activity);if(a)openSheet("activity",{...a,detail:a.payload?.answer||a.detail})})}

function showAIResult(result,source={type:"ai",name:""}){
  const target=resultTarget();target.innerHTML="";target.classList.remove("error");
  const title=document.createElement("div");title.style.cssText="white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.65";title.textContent=result?.summary||"Analisi completata";target.appendChild(title);
  ...
  (result?.actions||[]).filter(action=>action.kind!=="none").slice(0,4).forEach(action=>{const b=document.createElement("button");b.textContent=action.label||"Azione";b.style.cssText=save.style.cssText;b.onclick=()=>confirmAndExecuteAction(action,result);wrap.appendChild(b)});
  const copy=document.createElement("button");copy.textContent="Copia risposta";copy.style.cssText=save.style.cssText;copy.onclick=()=>executeAction({kind:"copy",label:"Copia risposta",payload:{text:result.summary}},result);wrap.appendChild(copy);
  const download=document.createElement("button");download.textContent="Scarica testo";download.style.cssText=save.style.cssText;download.onclick=()=>executeAction({kind:"download",label:"Scarica risposta",payload:{content:result.summary,filename:"risposta-one.txt",mime:"text/plain;charset=utf-8"}},result);wrap.appendChild(download);
...
async function askONE(payload,source={type:"ai",name:""}){
  ...
  try{
    ...
    setState("idle");const msg=(err?.message||"").toLowerCase(),code=err?.code||"";
    if(code==="insufficient_quota"||msg.includes("credits")||msg.includes("billing")||msg.includes("quota")){setAIStatus(false,"AI in pausa");show("ONE è operativo, ma il motore AI è temporaneamente in pausa per il credito API.",true)}
    else{setAIStatus(false,"AI non disponibile");show(err?.status===401||msg==="session_expired"?"Sessione scaduta. Accedi nuovamente a ONE.":code?(err.message||"Risposta AI non disponibile."):"Non riesco a contattare il motore AI in questo momento.",true)}
  }
}
...
$("askForm").onsubmit=async e=>{e.preventDefault();const v=promptInput.value.trim();if(!v)return;promptInput.value="";updateSend();await askONE({text:v},{type:"text",name:"richiesta"})};
...
function localDateKey(value=new Date()){const d=value instanceof Date?value:new Date(value);if(Number.isNaN(d.getTime()))return"";const pad=n=>String(n).padStart(2,"0");return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`}
function siteName(siteId){const site=sitesCache.find(item=>item.id===siteId);return site?`${site.job_number} · ${site.name}`:"Cantiere"}
function isClosedStatus(value=""){return ["risolto","risolta","chiusa","chiuso","completata","completato"].includes(String(value).trim().toLowerCase())}
function bindTodayItems(){$$("[data-today-site]").forEach(button=>button.onclick=()=>openSite(button.dataset.todaySite));$$('[data-today-action="reminders"]').forEach(button=>button.onclick=()=>openSheet("actions"))}
function showTodayLoggedOut(){const stateEl=$("todayState"),metrics=$("todayMetrics"),list=$("todayList");stateEl.classList.remove("hidden");metrics.classList.add("hidden");list.classList.add("hidden");stateEl.innerHTML=`<div class="today-login-copy"><strong>Il tuo briefing operativo</strong><span>Accedi per vedere priorità, scadenze e cantieri da aggiornare.</span></div><button id="todayLogin">Accedi</button>`;$("todayLogin").onclick=()=>openAuth()}
async function loadToday(){
  const stateEl=$("todayState"),metrics=$("todayMetrics"),list=$("todayList");if(!stateEl)return;
  if(!session){showTodayLoggedOut();return}
  stateEl.classList.remove("hidden");metrics.classList.add("hidden");list.classList.add("hidden");stateEl.innerHTML=`<div class="today-login-copy"><strong><span class="sync-spinner"></span> ONE sta preparando il briefing</strong><span>Controllo priorità, scadenze e aggiornamenti.</span></div>`;
  try{
    const today=localDateKey(),tomorrow=new Date();tomorrow.setDate(tomorrow.getDate()+1);tomorrow.setHours(23,59,59,999);
    const [sites,issues,reminders,reports]=await Promise.all([
      cloudRequest("sites?select=id,job_number,name,client,address,status,progress,updated_at&order=updated_at.desc"),
      cloudRequest("issues?select=id,site_id,title,priority,status,due_at,created_at&order=created_at.desc&limit=160"),
      cloudRequest("one_reminders?select=id,site_id,title,note,due_at,completed&completed=eq.false&order=created_at.desc&limit=60"),
      cloudRequest(`daily_reports?select=id,site_id,report_date&report_date=eq.${today}`)
    ]);
    sitesCache=Array.isArray(sites)?sites:[];const openIssues=(Array.isArray(issues)?issues:[]).filter(item=>!isClosedStatus(item.status));
    const urgent=openIssues.filter(item=>["critica","alta"].includes(String(item.priority||"").toLowerCase()));
    const overdue=openIssues.filter(item=>item.due_at&&new Date(item.due_at)<new Date());
    const dueReminders=(Array.isArray(reminders)?reminders:[]).filter(item=>item.due_at&&new Date(item.due_at)<=tomorrow);
    const reportedSites=new Set((Array.isArray(reports)?reports:[]).map(item=>item.site_id));
    const activeSites=sitesCache.filter(site=>!["chiuso","completato","archiviato"].includes(String(site.status||"").toLowerCase()));
    const missingReports=activeSites.filter(site=>!reportedSites.has(site.id));
    const items=[];
    [...overdue,...urgent.filter(item=>!overdue.some(x=>x.id===item.id))].slice(0,5).forEach(issue=>items.push({icon:issue.due_at&&new Date(issue.due_at)<new Date()?"⏱":"!",title:issue.title,detail:`${siteName(issue.site_id)} · ${issue.priority||"Priorità"}`,tag:issue.due_at&&new Date(issue.due_at)<new Date()?"SCADUTO":"URGENTE",siteId:issue.site_id}));
    dueReminders.slice(0,2).forEach(item=>items.push({icon:"◷",title:item.title,detail:item.due_at?new Date(item.due_at).toLocaleString("it-IT"):item.note||"Promemoria",tag:"PROMEMORIA",action:"reminders"}));
    missingReports.slice(0,2).forEach(site=>items.push({icon:"▤",title:`Aggiornamento ${site.job_number}`,detail:`${site.name} · report di oggi non presente`,tag:"DA AGGIORNARE",siteId:site.id}));
    stateEl.classList.add("hidden");metrics.classList.remove("hidden");list.classList.remove("hidden");
    metrics.innerHTML=`<div class="today-metric urgent"><strong>${urgent.length}</strong><span>priorità alte</span></div><div class="today-metric due"><strong>${overdue.length+dueReminders.length}</strong><span>scadenze</span></div><div class="today-metric"><strong>${missingReports.length}</strong><span>cantieri da aggiornare</span></div>`;
    list.innerHTML=items.length?items.map(item=>`<button class="today-item" ${item.siteId?`data-today-site="${item.siteId}"`:`data-today-action="${item.action}"`}><span>${item.icon}</span><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.detail)}</small></div><b>${item.tag}</b></button>`).join(""):`<div class="today-state card"><div class="today-login-copy"><strong>Nessuna urgenza rilevata</strong><span>ONE non vede scadenze o priorità critiche per oggi.</span></div><span style="color:#79e7c7">✓</span></div>`;
    bindTodayItems()
  }catch{stateEl.classList.remove("hidden");metrics.classList.add("hidden");list.classList.add("hidden");stateEl.innerHTML=`<div class="today-login-copy"><strong>Briefing non disponibile</strong><span>Controlla la connessione o aggiorna tra poco.</span></div><button id="retryToday">Riprova</button>`;$("retryToday").onclick=loadToday}
}
...
$("siteAskForm").onsubmit=async e=>{e.preventDefault();const input=$("sitePromptInput"),value=input.value.trim();if(!value||!selectedSiteId)return;input.value="";await askONE({text:value,site_id:selectedSiteId},{type:"site",name:$("siteTitle").textContent})};
$$("[data-site-action]").forEach(button=>button.onclick=()=>openOperationReview({kind:button.dataset.siteAction,label:workspaceActionTitle(button.dataset.siteAction),payload:{site_id:selectedSiteId}},{summary:`Operazione per ${$("siteTitle").textContent}`}));
$("refreshToday").onclick=loadToday;$("todayLogin").onclick=()=>openAuth();
...
function openSheet(type,data={}){
  ...
  const t={
    ...
    examples:()=>{title.textContent="Esempi pronti";sub.textContent="Dati demo nel tuo spazio";body.innerHTML=`<div class="legal-note">Scegli un pacchetto e controlla l'anteprima. I dati vengono creati solo dopo la tua conferma e restano separati per account.</div>${Object.entries(EXAMPLE_PACKS).map(([id,pack])=>`<article class="example-pack"><div class="example-pack-head"><span class="example-pack-icon">${pack.icon}</span><div class="example-pack-copy"><strong>${escapeHtml(pack.name)}</strong><small>${escapeHtml(pack.description)}</small></div></div><ul><li>1 spazio di lavoro</li><li>${pack.activities.length} attività e ${pack.issues.length} ${pack.issues.length===1?"criticità":"criticità"}</li><li>1 giornale lavori di esempio</li></ul><button class="example-install" data-example-pack="${id}">Installa esempio</button></article>`).join("")}<div id="exampleStatus" class="operation-status hidden"></div>`;setTimeout(bindExamplePacks,0)},
    about:()=>{title.textContent="ONE";sub.textContent="Versione 0.9 Pulse Beta";body.innerHTML=`<div style="text-align:center;padding:20px"><div class="recall-orb"><span></span></div><h3>Show it. Say it. Done.</h3><p>ONE 0.9 trasforma richieste, foto e voce in operazioni confermate e tracciate nei cantieri.</p></div>`},
    activity:()=>{title.textContent=data.title||"Attività";sub.textContent=data.time||"";body.innerHTML=`<div class="activity-card"><span class="a-icon">${data.icon||"✦"}</span><span class="a-copy"><strong>${escapeHtml(data.title||"")}</strong><small style="white-space:pre-wrap;overflow-wrap:anywhere">${escapeHtml(data.detail||"")}</small></span></div><p>${data.cloud?"Sincronizzata sul cloud ONE.":"Memorizzata sul dispositivo."}</p>`}
...
function updateAccountUI(){
  ...
  if(logged)syncFromCloud();if($("spacesView")?.classList.contains("active-view"))loadWorkspaces();loadToday()
}
...
function bindExamplePacks(){$$("[data-example-pack]").forEach(button=>button.onclick=()=>installExamplePack(button.dataset.examplePack,button))}
function dueDateFromNow(days){return new Date(Date.now()+days*86400000).toISOString()}
async function installExamplePack(packId,button){const pack=EXAMPLE_PACKS[packId],status=$("exampleStatus");if(!pack)return;if(!session){closeSheet();openAuth();toast("Accedi per installare un esempio");return}if(!confirm(`Installare “${pack.name}” nel tuo spazio ONE?\n\nVerranno creati un cantiere demo e i relativi dati. Potrai eliminarli in seguito.`))return;let createdSiteId=null;button.disabled=true;status.classList.remove("hidden","error");status.textContent="Installazione dell'esempio…";try{const stamp=Date.now().toString().slice(-6),owner=userId(),rows=await cloudRequest("sites",{method:"POST",body:{job_number:`DEMO-${packId.toUpperCase().slice(0,3)}-${stamp}`,name:pack.site.name,client:pack.site.client,address:pack.site.address,status:"Attivo",progress:pack.site.progress,notes:pack.site.notes,created_by:owner}}),site=rows?.[0];if(!site?.id)throw new Error("Il cantiere demo non è stato creato");createdSiteId=site.id;await Promise.all([cloudRequest("activities",{method:"POST",body:pack.activities.map(item=>({...item,site_id:site.id,created_by:owner}))}),cloudRequest("issues",{method:"POST",body:pack.issues.map(item=>({site_id:site.id,title:item.title,details:item.details,priority:item.priority,status:"Aperto",due_at:dueDateFromNow(item.dueDays),created_by:owner}))}),cloudRequest("daily_reports",{method:"POST",body:{...pack.report,site_id:site.id,report_date:localDateKey(),created_by:owner}})]);sitesCache=[];await loadWorkspaces();await loadToday();await addActivity(`${pack.name} installato`,site.name,"✦","example_pack",{pack:packId,site_id:site.id});const body=$("sheetBody");body.innerHTML=`<div class="example-success"><span class="example-pack-icon">✓</span><h3>Esempio installato</h3><p>${escapeHtml(site.name)} è pronto con attività, criticità e giornale lavori.</p><button class="primary-btn" id="openInstalledExample">Apri il cantiere demo</button></div>`;$("openInstalledExample").onclick=()=>{closeSheet();openSite(site.id)}}catch(error){if(createdSiteId)try{await cloudRequest(`sites?id=eq.${encodeURIComponent(createdSiteId)}`,{method:"DELETE"})}catch{}status.textContent=error?.message||"Installazione non riuscita";status.classList.add("error");button.disabled=false}}


function workspaceActionTitle(kind){return({create_site:"Nuovo cantiere",create_issue:"Nuovo problema",create_activity:"Registra attività",create_daily_report:"Report giornaliero",update_site_progress:"Aggiorna avanzamento"})[kind]||"Operazione cantiere"}
async function ensureSitesCache(){if(sitesCache.length)return sitesCache;const rows=await cloudRequest("sites?select=id,job_number,name,client,address,status,progress,updated_at&order=updated_at.desc");sitesCache=Array.isArray(rows)?rows:[];return sitesCache}
function dateTimeLocalValue(value){if(!value)return"";const d=new Date(value);if(Number.isNaN(d.getTime()))return String(value).slice(0,16);const local=new Date(d.getTime()-d.getTimezoneOffset()*60000);return local.toISOString().slice(0,16)}
function operationFields(action,siteId){const p=action.payload||{},siteOptions=sitesCache.map(site=>`<option value="${site.id}" ${site.id===siteId?"selected":""}>${escapeHtml(site.job_number)} · ${escapeHtml(site.name)}</option>`).join(""),site=`<label>Cantiere<select name="site_id" required><option value="">Seleziona il cantiere</option>${siteOptions}</select></label>`;
  if(action.kind==="create_site")return `<label>Numero commessa<input name="job_number" maxlength="80" required value="${escapeHtml(p.site_job_number||"")}"></label><label>Nome cantiere<input name="name" maxlength="180" required value="${escapeHtml(p.site_name||p.title||"")}"></label><label>Cliente<input name="client" maxlength="180" value="${escapeHtml(p.client||"")}"></label><label>Indirizzo<input name="address" maxlength="500" value="${escapeHtml(p.address||"")}"></label><label>Note<textarea name="notes" maxlength="1600">${escapeHtml(p.notes||"")}</textarea></label>`;
  if(action.kind==="create_issue")return site+`<label>Titolo<input name="title" maxlength="180" required value="${escapeHtml(p.title||p.text||"")}"></label><label>Descrizione<textarea name="details" maxlength="1600">${escapeHtml(p.details||p.note||p.body||"")}</textarea></label><div class="operation-grid"><label>Priorità<select name="priority"><option ${p.priority==="Bassa"?"selected":""}>Bassa</option><option ${p.priority==="Media"||!p.priority?"selected":""}>Media</option><option ${p.priority==="Alta"?"selected":""}>Alta</option><option ${p.priority==="Critica"?"selected":""}>Critica</option></select></label><label>Scadenza<input name="due_at" type="datetime-local" value="${dateTimeLocalValue(p.due_at||p.date)}"></label></div>`;
  if(action.kind==="create_activity")return site+`<label>Attività<input name="title" maxlength="180" required value="${escapeHtml(p.title||p.text||"")}"></label><label>Note<textarea name="notes" maxlength="1600">${escapeHtml(p.note||p.details||p.body||"")}</textarea></label>`;
  if(action.kind==="create_daily_report")return site+`<label>Data<input name="report_date" type="date" required value="${escapeHtml(p.report_date||p.date||localDateKey())}"></label><label>Riepilogo<input name="summary" maxlength="500" value="${escapeHtml(p.summary||p.title||"")}"></label><label>Lavorazioni<textarea name="works" maxlength="2400">${escapeHtml(p.works||p.text||"")}</textarea></label><label>Blocchi e criticità<textarea name="blockers" maxlength="1600">${escapeHtml(p.blockers||p.details||"")}</textarea></label><div class="operation-grid"><label>Persone<input name="workers" type="number" min="0" step="1" value="${escapeHtml(p.workers??"")}"></label><label>Ore<input name="hours" type="number" min="0" step="0.5" value="${escapeHtml(p.hours??"")}"></label></div>`;
  return site+`<label>Avanzamento<input name="progress" type="number" min="0" max="100" step="1" required value="${escapeHtml(p.progress??sitesCache.find(site=>site.id===siteId)?.progress??0)}"></label>`}
async function saveNewSite(data){
  if(!session)throw new Error("Accedi per creare un cantiere");
  const jobNumber=String(data.job_number||"").trim(),name=String(data.name||"").trim();
  if(!jobNumber||!name)throw new Error("Inserisci numero commessa e nome cantiere");
  if(jobNumber.length>80||name.length>180)throw new Error("Numero commessa o nome troppo lungo");
  const existing=await cloudRequest(`sites?select=id&job_number=eq.${encodeURIComponent(jobNumber)}&limit=1`);
  if(existing?.length)throw new Error("Esiste già un cantiere accessibile con questo numero commessa");
  const rows=await cloudRequest("sites",{method:"POST",body:{job_number:jobNumber,name,client:String(data.client||"").trim()||null,address:String(data.address||"").trim()||null,notes:String(data.notes||"").trim()||null,status:"Attivo",progress:0,created_by:userId()}});
  const site=rows?.[0];if(!site?.id)throw new Error("Il salvataggio non è stato confermato. Controlla Spazi prima di riprovare.");
  sitesCache=[site,...sitesCache.filter(item=>item.id!==site.id)];
  await logAction({kind:"create_site",label:`Cantiere ${jobNumber}`,payload:{site_id:site.id,job_number:jobNumber,name}},"executed");
  try{await addActivity(`Cantiere ${jobNumber}`,name,"⌂","workspace_action",{kind:"create_site",site_id:site.id});await loadWorkspaces();await loadToday()}catch{toast("Cantiere salvato. Aggiorna Spazi per visualizzarlo.")}
  return `Cantiere ${jobNumber} · ${name}`;
}
async function saveWorkspaceOperation(kind,data){if(kind==="create_site")return saveNewSite(data);const siteId=String(data.site_id||"");if(!siteId||!sitesCache.some(site=>site.id===siteId))throw new Error("Seleziona un cantiere accessibile");let label="";
  if(kind==="create_issue"){label=String(data.title||"").trim();if(!label)throw new Error("Inserisci il titolo del problema");await cloudRequest("issues",{method:"POST",body:{site_id:siteId,title:label,details:String(data.details||"").trim()||null,priority:String(data.priority||"Media"),status:"Aperto",due_at:data.due_at?new Date(data.due_at).toISOString():null,created_by:userId()}})}
  else if(kind==="create_activity"){label=String(data.title||"").trim();if(!label)throw new Error("Inserisci l'attività");await cloudRequest("activities",{method:"POST",body:{site_id:siteId,title:label,notes:String(data.notes||"").trim()||null,created_by:userId()}})}
  else if(kind==="create_daily_report"){label=`Report ${data.report_date||localDateKey()}`;await cloudRequest("daily_reports",{method:"POST",body:{site_id:siteId,report_date:data.report_date||localDateKey(),summary:String(data.summary||"").trim()||null,works:String(data.works||"").trim()||null,blockers:String(data.blockers||"").trim()||null,workers:data.workers===""?0:Number(data.workers),hours:data.hours===""?0:Number(data.hours),created_by:userId()}})}
  else if(kind==="update_site_progress"){const progress=progressValue(data.progress);label=`Avanzamento ${progress}%`;await cloudRequest(`sites?id=eq.${encodeURIComponent(siteId)}`,{method:"PATCH",body:{progress,updated_at:new Date().toISOString()}});const cached=sitesCache.find(site=>site.id===siteId);if(cached)cached.progress=progress}
  else throw new Error("Operazione non supportata");
  await logAction({kind,label,payload:{...data,site_id:siteId}},"executed");await addActivity(label,siteName(siteId),"✓","workspace_action",{kind,site_id:siteId});await loadToday();if(selectedSiteId===siteId)await openSite(siteId);return label}
async function openOperationReview(action,context={}){const a=normalizeAction(action);if(!session){openAuth();toast("Accedi per salvare operazioni nel cantiere");return}try{await ensureSitesCache()}catch{toast("Non riesco a caricare i cantieri");return}if(!sitesCache.length&&a.kind!=="create_site"){toast("Non ci sono cantieri accessibili");return}const p=a.payload||{};let siteId=selectedSiteId||p.site_id||"";if(!sitesCache.some(site=>site.id===siteId)&&p.site_job_number)siteId=sitesCache.find(site=>String(site.job_number).toLowerCase()===String(p.site_job_number).toLowerCase())?.id||"";const title=$("sheetTitle"),sub=$("sheetSubtitle"),body=$("sheetBody");closeDrawer();title.textContent=workspaceActionTitle(a.kind);sub.textContent="Controlla i dati prima del salvataggio";body.innerHTML=`<div class="operation-summary">${escapeHtml(context?.summary||"ONE ha preparato un'operazione. Nulla verrà salvato finché non confermi.")}</div><form id="operationForm" class="operation-form">${operationFields(a,siteId)}<div id="operationStatus" class="operation-status hidden"></div><button class="primary-btn" type="submit">Conferma e salva</button><button class="operation-cancel" id="operationCancel" type="button">Annulla</button></form>`;$("sheet").classList.add("open");$("sheet").setAttribute("aria-hidden","false");$("scrim").classList.remove("hidden");$("operationCancel").onclick=closeSheet;$("operationForm").onsubmit=async event=>{event.preventDefault();const form=event.currentTarget,status=$("operationStatus"),submit=form.querySelector('[type="submit"]'),data=Object.fromEntries(new FormData(form).entries());status.classList.remove("hidden","error");status.textContent="Salvataggio nel cantiere…";submit.disabled=true;try{const saved=await saveWorkspaceOperation(a.kind,data);status.textContent=`✓ ${saved} salvato`;setTimeout(()=>{closeSheet();toast("Operazione salvata nel cantiere")},650)}catch(error){status.textContent=error?.message||"Operazione non riuscita";status.classList.add("error");submit.disabled=false}}}

function actionIcon(k){return({reminder:"◷",calendar:"▣",email:"✉",call:"☎",maps:"⌖",share:"↗",copy:"⧉",download:"⇩",search:"⌕",open_url:"↗",sms:"◌",whatsapp:"◉",save:"⌁",create_site:"⌂",create_issue:"!",create_activity:"✓",create_daily_report:"▤",update_site_progress:"%"})[k]||"⚡"}
...
function confirmAndExecuteAction(action,context={}){const a=normalizeAction(action);if(WORKSPACE_ACTION_KINDS.has(a.kind)){openOperationReview(a,context);return}if(["email","call","maps","calendar","open_url","sms","whatsapp","search"].includes(a.kind)&&!confirm(`${a.label}\n\n${actionDescription(a)}\n\nVuoi continuare?`)){logAction(a,"cancelled");return}executeAction(a,context)}
